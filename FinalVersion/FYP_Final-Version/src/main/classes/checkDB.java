		...
		try {
			...
			try {
				if (!connection.isClosed()) {
					...
					statusBar.setText("Could not to the database!\n");
					//statusBar.setText(connection.toString());
					connected = false;
				}
			} catch (Exception e) {
			...
			// statusBar.setText("Could not find the database driver " +
			// e.getMessage());
		}
	}
	...
	public void checkData(String reg) {
		...
		try {
			...
			ResultSet results = statement
					.executeQuery("SELECT * FROM userdata WHERE RegistrationPlate = '" + reg + "'");
			...
			while (results.next()) {
				...
				// column data are in the VARCHAR format

				String data = results.getString(1);

				// System.out.println("Fetching data by column index for row " +
				// results.getRow() + " : " + data);
				...
				// "+results.getString("RegistrationPlate")+"\nLast Activity:
				// "+results.getString("LastActivity")+" Accessed Front Gate";
				...
				// System.out.println("Fetching data by column name for row " +
				// results.getRow() + " : " + data);
